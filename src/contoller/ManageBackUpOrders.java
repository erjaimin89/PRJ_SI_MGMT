...
public class ManageBackUpOrders {
	...
	public List<ThresholdDetailEntity> fetchThresholdDetails(long itemId) {
		...
		try {
			tx = session.beginTransaction();
			Query query = session.createQuery("FROM ThresholdDetailEntity where itemId = :itemId order by itemDetailId");
			query.setParameter("itemId", itemId);
			backupOrders = (List<ThresholdDetailEntity>) query.list();
			tx.commit();
		} catch (HibernateException e) {
			if (tx != null)
				tx.rollback();
	...
	public boolean generateBackUpOrder(ComboItem selectedParty, Long arrvDate, String remarks) {
		...
		if(!detailsByParty.isEmpty()){
			...
			if(!detailIds.isEmpty()){
					...
					for(ThresholdDetailEntity thresholdDetail : thresholdDetails){
						for(ProductDetailEntity productDetail : productDetails){
							if(thresholdDetail.getItemDetailId() == productDetail.getItemDetailId()){
								...
								xQty = (xQty > 0)?xQty:0;
								if((productDetail.getXsmall() + productDetail.getTrans_xsmall() + xQty) <= thresholdDetail.getXsmallThreshold()){
									xsmall = thresholdDetail.getXsmallTarget();
									flag = true;
								}
								int sQty = productDetail.getProd_small() - productDetail.getTrans_small();
								sQty = (sQty > 0)?sQty:0;
								if((productDetail.getSmall() + productDetail.getTrans_small() + sQty) <= thresholdDetail.getSmallThreshold()){
									small = thresholdDetail.getSmallTarget();
									flag = true;
								}
								int mQty = productDetail.getProd_medium() - productDetail.getTrans_medium();
								mQty = (mQty > 0)?mQty:0;
								if((productDetail.getMedium() + productDetail.getTrans_medium() + mQty) <= thresholdDetail.getMediumThreshold()){
									medium = thresholdDetail.getMediumTarget();
									flag = true;
								}
								int lQty = productDetail.getProd_large() - productDetail.getTrans_large();
								lQty = (lQty > 0)?lQty:0;
								if((productDetail.getLarge() + productDetail.getTrans_large() + lQty) <= thresholdDetail.getLargeThreshold()){
								...
								if((productDetail.getXlarge() + productDetail.getTrans_xlarge() + xlQty) <= thresholdDetail.getXlargeThreshold()){
								...
								if((productDetail.getXxlarge() + productDetail.getTrans_xxlarge() + xxlQty) <= thresholdDetail.getXxlargeThreshold()){
								...
								if((productDetail.getXxxlarge() + productDetail.getTrans_xxxlarge() + xxxlQty) <= thresholdDetail.getXxxlargeThreshold()){
								...
								if((productDetail.getMix() + productDetail.getTrans_mix() + mixQty) <= thresholdDetail.getMixThreshold()){
									mix = thresholdDetail.getMixTarget();
									flag = true;
								}
								if(flag){
									int detailTotalQuantity = xsmall + small + medium + 
											large + xlarge + xxlarge + xxxlarge + mix;
									itemTotalQuantity += detailTotalQuantity;
									itemDetails.add(new BackUpOrderDetailEntity
											(selectedParty.getKey(), selectedParty.getValue(), itemId, fetchProductEntity.getItemName(), 
													productDetail.getItemDetailId(), productDetail.getName(), xsmall, small, medium, 
													large, xlarge, xxlarge, xxxlarge, mix, detailTotalQuantity));
								}
								break;
							}
						}
				...
				if(!backUpOrderDetails.isEmpty()){
					BackUpOrderEntity backUpEntity = new BackUpOrderEntity(totalQuantity, arrvDate, BackUpOrderStatus.NOT_SENT.ordinal(), Utils.getDateInMillies(new Date()), 
	...
	public List<BackUpOrderEntity> listOrders() {
		...
		try {
			tx = session.beginTransaction();
			orders = (List<BackUpOrderEntity>)session.createQuery(
					"FROM BackUpOrderEntity order by backUpOrderId desc").list();
			tx.commit();
		} catch (HibernateException e) {
			if (tx != null)
				tx.rollback();
	...
	public boolean deleteOrder(Long backUpOrderId) {
		...
		try {
			tx = session.beginTransaction();
			BackUpOrderEntity myObject = (BackUpOrderEntity)session.load(BackUpOrderEntity.class,backUpOrderId);
	...
	@SuppressWarnings("unchecked")
	public List<BackUpOrderEntity> listOrdersBySearch(String orderNo,
		...
		try {
			...
			orders = (List<BackUpOrderEntity>) query.list();
			tx.commit();
		} catch (HibernateException e) {
			if (tx != null)
				tx.rollback();
	...
	public boolean updateBackUpOrder(BackUpOrderEntity backUpOrderEntity,
			boolean shippedFlag) {
		Session session = factory.openSession();
		Transaction tx = null;
		try {
			...
			if(shippedFlag){
				...
				for(BackUpOrderDetailEntity orderDetail : orderDetails){
					...
					ProductEntity productEntity = session.get(ProductEntity.class, itemId);
					if(!itemSet.contains(itemId)){
						productEntity.setProductionQuantity(productEntity.getProductionQuantity() + orderDetail.getItemTotalQuantity());
						productEntity.setUpdatedOn(System.currentTimeMillis());
						itemSet.add(itemId);
					}
					for(ProductDetailEntity detailEntity : productEntity.getDetails()){
						if(detailEntity.getItemDetailId() == orderDetail.getItemDetailId()){
							...
							detailEntity.setTotal_production(detailEntity.getTotal_production() + orderDetail.getDetailTotalQuantity());
							break;
						}
					}
					session.update(productEntity);
