...
public class AddUserDialog extends JDialog {
	...
	private static final long serialVersionUID = -7692961309756815650L;
				...
				if (user.length() != 0) {
					if (!manageUsers.isUserExists(user)) {
						boolean addUser = manageUsers.addUser(new UserEntity(
								user, System.currentTimeMillis(), UserType.USER.ordinal()));
						if (addUser) {
							...
							String message = "Error while adding a new user !";
							JOptionPane.showMessageDialog(new JFrame(),
									message, "Dialog",
									JOptionPane.ERROR_MESSAGE);
						}
					} else {
						String message = "User with same name already exists !";
						JOptionPane.showMessageDialog(new JFrame(), message,
								"Dialog", JOptionPane.ERROR_MESSAGE);
					}
				} else {
					String message = "The User Name Field must be filled in !";
					JOptionPane.showMessageDialog(new JFrame(), message,
							"Dialog", JOptionPane.ERROR_MESSAGE);
	...
	private void loadUsers() {
		...
		List<UserEntity> list = manageUsers.listUsers(UserType.USER.ordinal());
